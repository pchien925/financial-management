...
function Finance() {
  ...
  // hàm này được dùng để cập nhật searchParams một cách tiện lợi, có thể xóa param khi giá trị là null/undefined/empty
  ...
  //hàm lọc giao dịch dựa trên các tham số tìm kiếm và lọc
  const filteredTransactions = useMemo(() => {
    return transactions.filter((item) => {
      const user = users.find(u => u.id === item.userId);
      const userName = user ? user.name.toLowerCase() : '';

      const matchSearch = item.note.toLowerCase().includes(searchText.toLowerCase()) || userName.includes(searchText.toLowerCase());
  ...
  }, [transactions, users, searchText, filterType, filterCategory, startDate, endDate]);
  ...
  const handleConfirmExport = async () => {
    ...
    // Lọc dataToExport từ filteredTransactions (giữ nguyên các bộ lọc hiện tại, chỉ thu hẹp theo tháng)
  ...
  const handleEdit = (record) => {
    navigate(`/transactions/edit/${record.id}?${searchParams.toString()}`);
  };
